...
//  FSubMdiChild,
  StdCtrls, ExtCtrls, Grids, DBGrids, ComCtrls, Buttons, DBase,
  Gradpan, ExtPage, DBCtrls, //FReports,
  Globals, Db, ExtDlgs, Menus,
  FSearch, ActnList, StrUtils, Variants, ToolWin, ImgList, RBaseADO,
  FDefineQOrder, FFilterEn, ADODB;

type
  TfrmBase = class(TfrmSubMdiChild)
	panTop: TGradPan;
...
//        function  FindFieldsForIndex(LeoMenu:TMenuItem):string;
//        function  SetQIndexFields(LeoMenu:TMenuItem):string;
//   	  function GetDM : TDMBase; virtual;
  public
        PFormName  : String;
...
//	property Visible : boolean read GetFormVisible write SetFormVisible;
...
constructor TfrmBase.CreateModal( aowner:TComponent;FormID:Integer);
begin
        Created := False;
        IndexAct:= False;
	FModal  := True;
...
constructor TfrmBase.CreateSecurity(aowner:TComponent;FormID:Integer);
begin
        Created := False;
        IndexAct:= False;
        PFormID := FormID;
...
procedure TfrmBase.SetState(const LeoCategory:String);
var i:Integer;
    LeoS:String;
begin
...
procedure TfrmBase.PlusState(const LeoCategory:String);
var i:Integer;
begin
for i:= 0 to ActionList1.ActionCount -1  do
  if ActionList1.Actions[i].Category=LeoCategory then
...
procedure TfrmBase.DataModuleStateChange( Sender:TObject );
begin
//
end;

procedure TfrmBase.SelectImage( dlg:TOpenPictureDialog ; field:TBlobField );
var
	stream : TBlobStream;
	image : TPicture;
	ds : TDataSet;
...
procedure TfrmBase.SetFormStyle( value:TFormStyle );
begin
	if FModal then
...
function TfrmBase.GetFormStyle : TFormStyle;
begin
	Result := inherited FormStyle;
end;

procedure TfrmBase.SetFormVisible( value:boolean );
begin
	if FModal then
...
function TfrmBase.GetFormVisible : boolean;
begin
	Result := inherited Visible;
end;

procedure TfrmBase.SetPosition( value:TPosition );
begin
	if FModal then
...
function TfrmBase.GetPosition : TPosition;
begin
	Result := inherited Position;
end;

procedure TfrmBase.AdjustDBNavigator;
var
	n, i : Integer;
begin
...
function TfrmBase.DoOnPageChange : boolean;
begin
	Result := True;
...
procedure TfrmBase.btnDetailClick(Sender: TObject);
begin
  inherited;
	if (DM <> nil) then
           DM.BrowseData;
...
procedure TfrmBase.dgDataDrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
var
	dg : TDbGrid;
begin
...
function TfrmBase.AskDelete : boolean;
begin
	Result := sAlert( mtConfirmation, SCADeleteRecord, 2 ) = 1;
end;

procedure TfrmBase.pmsFilterClick(Sender: TObject);
...
procedure TfrmBase.BBAccClick(Sender: TObject);
begin
  DM.UpdateData;
  DM.OnStateChange(Sender);
end;

procedure TfrmBase.BBCancelClick(Sender: TObject);
begin
  DM.CancelData;
  DM.OnStateChange(Sender);
end;

procedure TfrmBase.pmsCancelFilterClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmBase.dgDataEnter(Sender: TObject);
begin
  inherited;
Self.FilteredGrid:=Self.dgData;
   FilteredGrid:=dgData;
   BrowseIndex(Indexes);
   BrowseFields(Pedia);
...
procedure TfrmBase.N2Click(Sender: TObject);
var frm:TfrmSearch;
begin
...
procedure TfrmBase.popfilterDisClick(Sender: TObject);
begin
  inherited;
if popfilter.PopupComponent is TDBGrid then
...
procedure TfrmBase.InsertExecute(Sender: TObject);
begin
  inherited;
...
procedure TfrmBase.UpdateExecute(Sender: TObject);
begin
  inherited;
  FPageFlag := False;
  TogglePage;
...
procedure TfrmBase.DeleteExecute(Sender: TObject);
begin
  inherited;
...
procedure TfrmBase.actPrintExecute(Sender: TObject);
var
	frm : TfrmReports;
begin
  inherited;
...
procedure TfrmBase.PostExecute(Sender: TObject);
begin
  inherited;
  DM.UpdateData;
{ TODO : check ref }
...
procedure TfrmBase.CancelExecute(Sender: TObject);
begin
  inherited;
  DM.CancelData;
  DM.OnStateChange(Sender);
...
procedure TfrmBase.N4Click(Sender: TObject);
var rep:TrptSubParams;
begin
if ((popFilter.PopupComponent is TDBGrid )) then
    begin
    if (popFilter.PopupComponent is TDBGrid ) then
      if (popFilter.PopupComponent = dgData) then
...
procedure TfrmBase.DetailExecute(Sender: TObject);
begin
  inherited;
	if (DM <> nil) then
           DM.BrowseData;
...
procedure TfrmBase.ActionList1Execute(Action: TBasicAction;
  var Handled: Boolean);
begin
...
procedure TfrmBase.SetStatusText(Text:String;Index:Integer);
begin
...
procedure TfrmBase.AfterExecute(Sender: TObject);
var i:integer;
begin
  inherited;
inc(i);
...
procedure TfrmBase.BeforeExecute(Sender: TObject);
var i:integer;
begin
  inherited;
inc(i);
...
procedure TfrmBase.UpdateActionsExecute(Sender: TObject);
begin
  inherited;
  DMMain.UpdateActionsExecute(FormId,ActionList1);
end;

procedure TfrmBase.UpdateFieldsExecute(Sender: TObject);
var i:integer;
begin
  inherited;
...
frmMain.WriteGrid(PFormName,dgData);
DMMain.UpdSecurity(PFormName);
end;

procedure TfrmBase.BrowseFields(var LeoMenu:TMenuItem);
Var i,x    :Integer;
    NewMenu:TMenuItem;
begin
  x:=LeoMenu.Count -3;
...
procedure TfrmBase.BrowseFieldsIdx(var LeoMenu:TMenuItem);
Var i,x      :Integer;
    NewMenu:TMenuItem;
begin
  x:=LeoMenu.Count -2;
...
procedure TfrmBase.BrowseIndex(var LeoMenu:TMenuItem);
Var
  i,x           :Integer;
...
procedure TFrmBase.UncheckIndexes();
var i:integer;
begin
...
procedure TFrmBase.SecMenuTableOnClick(Sender: TObject);
begin
...
procedure TFrmBase.SecMenuQueryOnClick(Sender: TObject);
...
procedure TFrmBase.NLeoClick(Sender: TObject);
var Leo:String;
    x,y:Integer;
begin
...
procedure TFrmBase.NLeo1Click(Sender: TObject);
begin
If (Sender as TMenuItem).Checked then
  (Sender as TMenuItem).Checked:=False
else
...
procedure TFrmBase.FAllClick(Sender: TObject);
var i:integer;
begin
  For i:=0 to (TDBGrid(FilteredGrid)).Columns.Count -1 do
   begin
...
procedure TfrmBase.FNoneClick(Sender: TObject);
var i:Integer;
begin
  For i:=0 to (TDBGrid(FilteredGrid)).Columns.Count -1 do
  begin
...
procedure TfrmBase.NNoneClick(Sender: TObject);
...
procedure TfrmBase.IndDispClick(Sender: TObject);
var LeoS,SLeo:String;
    i,z:integer;
    LeoV:Variant;
begin
...
procedure TfrmBase.PediaIdxActiveClick(Sender: TObject);
...
function TfrmBase.FindFieldsForIndex(LeoMenu:TMenuItem):string;
var LeoS:String;
    i:Integer;
    found:Boolean;
begin
...
procedure TfrmBase.NextExecute(Sender: TObject);
begin
  inherited;
(TDBGrid(FilteredGrid)).DataSource.DataSet.Next;
end;

procedure TfrmBase.PreviousExecute(Sender: TObject);
begin
  inherited;
(TDBGrid(FilteredGrid)).DataSource.DataSet.Prior;
end;

procedure TfrmBase.dgDataColumnMoved(Sender: TObject; FromIndex,
  ToIndex: Integer);
Var SLeo:String;
    NewMenu:TMenuItem;
    i:Integer;
...
procedure TfrmBase.CloseExecute(Sender: TObject);
begin
  inherited;
...
function TfrmBase.SetQIndexFields(LeoMenu: TMenuItem): string;
var LeoS:String;
    i,x,y:Integer;
    found:Boolean;
...
procedure TfrmBase.ModalOkExecute(Sender: TObject);
begin
  inherited;
 ModalResult:=mrOk;
end;

procedure TfrmBase.ModalCancelExecute(Sender: TObject);
begin
  inherited;
 ModalResult:=mrCancel;
end;

procedure TfrmBase.SetGrid(Grid: TComponent; OnOff: Boolean);
begin
...
procedure TfrmBase.dgDataTitleClick(Column: TColumn);
...
procedure TfrmBase.ReturnAdoParameters(ParametersArray,
...
procedure TfrmBase.SaveAdoParameters(var ParametersArray,
  ValuesArray: Variant; CountParameters: Integer; Query: TAdoQuery);
var x:Integer;
begin
  for x:=0 to (CountParameters-1) do
...
procedure TfrmBase.actExportToXlClick(Sender: TObject);
begin
  inherited;
  TDMBase(DMMain).OutputFile(TDBGrid(FilteredGrid));
end;

procedure TfrmBase.actPrintFormExecute(Sender: TObject);
begin
  inherited;
...
constructor TfrmBase.CreateSecurityID(aowner: TComponent; FormID,
  MainId: Integer);
begin
  Created := False;
  IndexAct:= False;
