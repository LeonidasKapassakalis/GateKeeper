...
Type
  TfrmImportStdTrCall = Class(TfrmBaseDev0)
...
Procedure TfrmImportStdTrCall.actCloseExecute(Sender: TObject);
Begin
  Close;
End;

// Leonidas
Procedure TfrmImportStdTrCall.dxButtonEdit1ButtonClick(Sender: TObject; AbsoluteIndex: Integer);
Begin
...
procedure TfrmImportStdTrCall.Edit1DblClick(Sender: TObject);
begin
  // OpenDialog1.InitialDir := ExtractFilePath(Application.ExeName);
  OpenDialog1.InitialDir := ExtractFilePath(Application.ExeName);
  // !!!!
...
Procedure TfrmImportStdTrCall.FillBinBD(BinBD: TBINBD);
var
...
Procedure TfrmImportStdTrCall.FillBinBP(BinBP: TBINBP);
Var
...
Procedure TfrmImportStdTrCall.FillBinBS(BinBP: TBINBP);
Begin
  SetReadState(2);
...
Procedure TfrmImportStdTrCall.FillBinBT(BinBP: TBINBP);
Begin
  SetReadState(4);
...
Procedure TfrmImportStdTrCall.FillBinHDR;
Begin
  SetReadState(99);
  // Leonidas Check
...
Procedure TfrmImportStdTrCall.Process(FileName: ShortString);
Var
  F: ShortString;
  Y, M, D, i, x: Integer;
  Found: Boolean;
...
Procedure TfrmImportStdTrCall.ReadFromStream;
Var
  i: Integer;
Begin
...
Function TfrmImportStdTrCall.ReadPosHDR(Const Year, Month, Day: Integer): Boolean;
Begin
  Result := true;
  try
...
Procedure TfrmImportStdTrCall.ReadStationData;
Var
  i: Integer;
  BinBP: TBINBP;
Begin
...
Procedure TfrmImportStdTrCall.ReadTransData(StationIdx: Integer);
Var
  BinBD: TBINBD;
Begin
  try
...
Procedure TfrmImportStdTrCall.FormCreate(Sender: TObject);
var
  x: Integer;
Begin
...
Procedure TfrmImportStdTrCall.FormDestroy(Sender: TObject);
Var
  BinBP: TBINBP;
  BinBD: TBINBD;
  i: Integer;
...
Function TfrmImportStdTrCall.GetPosString(a: ShortString): ShortString;
Begin
  Result := Trim(a);
End;

Function TfrmImportStdTrCall.GetPosInteger(a: ShortString): Integer;
Var
  tmp: ShortString;
  i: Integer;
Begin
...
Procedure TfrmImportStdTrCall.ReportError;
var
  x: String;
Begin
  Case ErrorID Of
...
Procedure TfrmImportStdTrCall.FormShow(Sender: TObject);
Begin
  Label2.Caption       := '';
...
Procedure TfrmImportStdTrCall.DoInsert;
                    ...
                    Begin
                      If CheckSitem(BinBD.ProductPurchased, SitemCode) = true then
                        begin
                          ...
                          // begin
                          // try
                          // qryTranUNITPRICE.AsFloat :=
                          // BinBD.TransactionAmount / BinBD.Quantity;
                          // except
                          // MessageDlg('Διαίρεση με Μηδεν.', mtError,
                          // [mbOK], 0);
                          // qryTranUNITPRICE.AsFloat := 0;
                          // end;
                          // CheckSitemPrice(SitemCode, MerchandCode,
                          // qryTranUNITPRICE.AsFloat);
                          // end;
                          // 18062008
                          // if Checking then
                          // if CheckSameEntry(CardCode, MerchandCode, SitemCode,
                          // qryTranTRANDATE.AsDateTime,
                          // qryTranTRANTIME.AsDateTime,
                          // BinBD.TransactionAmount,
...
procedure TfrmImportStdTrCall.actPrintExecute(Sender: TObject);
var
  frm: TQuickRep;
begin
  try
...
procedure TfrmImportStdTrCall.SetReadState(const Value: Integer);
begin
  FReadState := Value;
end;

procedure TfrmImportStdTrCall.DisplayErrorLine(Num: Integer = 0);
var
  tmp: Integer;
begin
  if Num > 0 then
...
function TfrmImportStdTrCall.POSBDToString(): ShortString;
var
  x: String;
begin
  x      := '';
...
function TfrmImportStdTrCall.POSBPToString: ShortString;
var
  x: String;
begin
  x      := '';
...
function TfrmImportStdTrCall.POSBSToString: ShortString;
var
  x: String;
begin
  x      := '';
...
function TfrmImportStdTrCall.POSBTToString: ShortString;
var
  x: String;
begin
  x      := '';
...
function TfrmImportStdTrCall.CheckSameEntry(Card, Merchand, Sitem: Integer; TTDate, TTTime: TDateTime; Amount: Double; RefStr: String): Boolean;
begin
...
function TfrmImportStdTrCall.MerchandName(Code: Integer): String;
var
  vCode: Variant;
begin
  // FindMerchand.Active                              := False;
...
function TfrmImportStdTrCall.CheckTrCallForRerun(DateTr: Variant; var BatchNo: Integer): Boolean;
begin
...
function TfrmImportStdTrCall.DeleteTrBatch(DateTr: Variant): Boolean;
begin
...
function TfrmImportStdTrCall.CheckIfTransUpdatePelStat(BatchNo: Integer): Boolean;
begin
...
function TfrmImportStdTrCall.UpdateTrCallForDate(CallDDate: TDateTime; var BatchNo: Integer): Boolean;
var
  CallVar: Variant;
begin
  qryTrCall.Insert;
