...
uses
  ...
  Grids, DBGrids, Stdctrls, Buttons, DB, dbTables, dbctrls, Globals, Globals0,
  Common, DMain, Variants, ADODB;

type
  TfrmSearch2 = class(TForm)
    BitBtn1: TBitBtn;
...
constructor TfrmSearch2.CreateWithParam(aowner: TComponent; grid: TDBGrid; DM: TDataModule);
begin
  FGrid  := grid;
  WithFP := False;
...
constructor TfrmSearch2.CreateWithParamVal(aowner: TComponent); // ;FTableName,FName,FValue:String);
begin
  // FieldNam := FName;
  // FieldVal := FValue;
  WithFP := True;
...
procedure TfrmSearch2.FillCombo;
var
...
function TfrmSearch2.CreateLCB(Owner: TForm; Name, LName: String; Width: Integer; LField: TFieldClass; NNField: String; Field: TField): TComponent;
var
...
function TfrmSearch2.CreateEdit(Owner: TForm; Name, LName: String; Width: Integer; LField: TFieldClass; NNField: String): TComponent;
var
  LEdit: TEdit;
  LLabel: TLabel;
begin
...
procedure TfrmSearch2.EditDateEnter(Sender: TObject);
var
  TmpDate: TDateTime;
begin
...
procedure TfrmSearch2.FormCreate(Sender: TObject);
begin
  if Not(FGrid = Nil) then
    FillCombo;
end;

procedure TfrmSearch2.BitBtn1Click(Sender: TObject);
var
  a, b: String;
  i, ii: Integer;
...
begin
  ...
  try
    ...
    if FGrid.DataSource.DataSet is TAdoTable then
      TblName := TAdoTable(FGrid.DataSource.DataSet).TableName;
    ...
    if frmlup.ADOQuery1.RecordCount = 1 then
      begin
        ...
        FGrid.DataSource.DataSet.Locate('id', vi, [loPartialKey]);
      end;
    if frmlup.ADOQuery1.RecordCount > 1 then
      begin
        frmlup.ShowModal;
        if frmlup.ModalResult = mrOk then
          begin
            vi := frmlup.ADOQuery1.FieldByName('Id').Value;
            vf := FGrid.DataSource.DataSet.Locate('Id', vi, []);
...
function TfrmSearch2.AutoSearch(FTableName, FName: String; var FValue: String; FKeyField: String = 'id'): Integer;
var
  a, b: String;
  i, ii: Integer;
  KSleo, TblName, SelFields: String;
